                            ...
                            <div class="table-responsive">
                                <table class="table text-nowrap mb-0 align-middle">
                                    ...
                                    <tbody>
                                        <?php
                                        $currDate = date("Y-m-d");
                                        $query = @mysqli_query($mysqli, "SELECT * FROM attendance WHERE is_present = 1 AND user_id = '$studentId'") or die(mysqli_error($mysqli));
                                        $count = mysqli_num_rows($query);

                                        while($row = mysqli_fetch_array($query)){
                                            $eventId = $row['event_id'];
                                            $events = @mysqli_query($mysqli, "SELECT * FROM events WHERE event_id = '$eventId'") or die(mysqli_error($mysqli));
                                            $evntsrow = mysqli_fetch_array($events);
                                            $organizername = @mysqli_query($mysqli, "SELECT first_name, last_name FROM users WHERE user_id = {$evntsrow['organizer_id']}") or die(mysqli_error($mysqli));
                                            ...
                                            echo "</td>";
                                            echo "<td class='border-bottom-0'>";
                                            echo "<h6 class='fw-semibold mb-1'>".$evntsrow['event_name']."</h6>";
                                            echo "</td>";
                                            echo "<td class='border-bottom-0'>";
                                            echo "<h6 class='fw mb-1'>".$evntsrow['event_description']."</h6>";
                                            echo "</td>";
                                            echo "<td class='border-bottom-0'>";
                                            echo "<h6 class='fw-semibold mb-1'>".$organizername['first_name']."  ".$organizername['last_name']."</h6>";
                                            echo "</td>";
                                            echo "<td class='border-bottom-0'>";
                                            echo "<h6 class='fw-semibold mb-1'>".$evntsrow['event_date']."</h6>";
                                            echo "</td>";
                                            echo "<td class='border-bottom-0'>";
                                            echo "<h6 class='fw-semibold mb-1'>".$evntsrow['event_start_time']."</h6>";
                                            echo "</td>";
                                            echo "<td class='border-bottom-0'>";
                                            echo "<h6 class='fw-semibold mb-1'>".$evntsrow['event_end_time']."</h6>";
                                            echo "</td>";
                                            echo "</tr>";
                                            $i++;
                                        }
